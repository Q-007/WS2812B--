...
LED_COUNT = 256  # 16x16 = 256个灯珠
MATRIX_WIDTH = 16
MATRIX_HEIGHT = 16
PATTERNS_FILE = "patterns.json"  # 图案保存文件
BRIGHTNESS = 0.05  # 亮度调节（0.0-1.0），建议16x16矩阵使用0.3-0.5
...
# 滚动文字相关变量
scroll_text_data = []  # 滚动文字的点阵数据
scroll_offset = 0  # 当前滚动偏移量
is_scrolling = False  # 是否正在滚动
scroll_speed = 1.0  # 滚动速度（秒）- 慢:1.0 快:0.005
...
def show_rainbow():
    ...
    for i in range(LED_COUNT):
        ...
        # 应用亮度调节
        r = int(colors[color_index][0] * BRIGHTNESS)
        g = int(colors[color_index][1] * BRIGHTNESS)
        b = int(colors[color_index][2] * BRIGHTNESS)
        np[i] = (r, g, b)
        led_colors[i] = (r, g, b)
    np.write()
...
def matrix_to_columns(text_data):
    """将16xN的0/1点阵(按行)转换为按列16位bitmask列表"""
    if not text_data or not isinstance(text_data, list):
        return []
    h = min(16, len(text_data))
    try:
        w = len(text_data[0]) if text_data else 0
    except:
        w = 0
    if w <= 0:
        return []
...
def display_scroll_frame(offset):
    """显示滚动文字的一帧（scroll_text_data为按列bitmask列表）"""
    global scroll_text_data
    if not scroll_text_data:
        return
    ...
    # scroll_text_data现在是按列的bitmask列表
    text_width = len(scroll_text_data)
    if text_width <= 0:
        np.write()
        return
    ...
    # 显示窗口为16列：col(0..15) 对应文字的 data_col = col + offset
    for col in range(16):
        data_col = col + offset
        if 0 <= data_col < text_width:
            mask = scroll_text_data[data_col]
            # 遍历16行，检查每一位
            for row in range(16):
                if mask & (1 << row):
                    led_index = row * 16 + col
                    np[led_index] = (r, g, b)
...
def start_scroll_text():
    """开始滚动文字"""
    global scroll_offset, is_scrolling
    scroll_offset = -16  # 从右侧空白开始进入
    is_scrolling = True
    print('开始滚动文字')
...
def get_html_page_compressed():
    ...
    html = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>LED控制</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial;background:linear-gradient(135deg,#667eea,#764ba2);padding:10px;user-select:none}.container{max-width:800px;margin:0 auto;background:#fff;border-radius:10px;padding:15px}h1{text-align:center;margin-bottom:15px;font-size:20px}.controls{margin-bottom:15px;padding:10px;background:#f5f5f5;border-radius:8px}label{display:block;margin:5px 0;font-weight:bold}input[type="color"],input[type="text"]{width:100%;height:40px;border:none;border-radius:5px;padding:0 10px}button{width:100%;padding:10px;margin:5px 0;border:none;border-radius:5px;font-size:14px;color:#fff;cursor:pointer}.btn-primary{background:#667eea}.btn-success{background:#48bb78}.btn-danger{background:#f56565}.btn-warning{background:#f59e0b}.btn-info{background:#3b82f6}.btn-purple{background:#9333ea}.led-matrix{display:grid;grid-template-columns:repeat(16,1fr);gap:2px;margin:15px 0;background:#2d3748;padding:10px;border-radius:8px;touch-action:none}.led{aspect-ratio:1;border-radius:50%;background:#000;cursor:crosshair;border:1px solid #4a5568}.preset-colors{display:grid;grid-template-columns:repeat(8,1fr);gap:3px;margin:10px 0}.preset-color{aspect-ratio:1;border-radius:5px;cursor:pointer;border:2px solid #ddd}.eraser-btn{background:linear-gradient(135deg,#f5f5f5,#e0e0e0);display:flex;align-items:center;justify-content:center;font-size:18px;border:2px solid #999}.pattern-section{margin-top:15px;padding:10px;background:#e8f5e9;border-radius:8px}.scroll-section{margin-top:15px;padding:10px;background:#fef3c7;border-radius:8px}.pattern-input{width:100%;padding:8px;border:1px solid #ddd;border-radius:5px;margin-bottom:8px}.pattern-list{max-height:150px;overflow-y:auto;margin-top:8px}.pattern-item{display:flex;justify-content:space-between;padding:6px;margin:3px 0;background:#fff;border-radius:5px}.pattern-item button{width:auto;padding:4px 8px;margin:0 2px;font-size:11px}.info{text-align:center;color:#666;margin:8px 0;font-size:12px}</style></head><body><div class="container"><h1>LED控制</h1><div class="controls"><label>选择颜色：</label><input type="color" id="colorPicker" value="#ff0000"><label>预设颜色：</label><div class="preset-colors"><div class="preset-color eraser-btn" onclick="selectEraser()">擦</div><div class="preset-color" style="background:#f00" onclick="selectPreset('#ff0000')"></div><div class="preset-color" style="background:#0f0" onclick="selectPreset('#00ff00')"></div><div class="preset-color" style="background:#00f" onclick="selectPreset('#0000ff')"></div><div class="preset-color" style="background:#ff0" onclick="selectPreset('#ffff00')"></div><div class="preset-color" style="background:#f0f" onclick="selectPreset('#ff00ff')"></div><div class="preset-color" style="background:#0ff" onclick="selectPreset('#00ffff')"></div><div class="preset-color" style="background:#fff" onclick="selectPreset('#ffffff')"></div></div><button class="btn-primary" onclick="setAllLeds()">设置全部</button><button class="btn-success" onclick="showRainbow()">彩虹效果</button><button class="btn-danger" onclick="clearAll()">清除</button></div><div class="scroll-section"><label>文字滚动：</label><input type="text" id="scrollText" class="pattern-input" placeholder="输入要滚动的汉字或文字..." maxlength="50"><div style="display:flex;gap:5px;margin-bottom:5px"><button class="btn-info" onclick="setScrollSpeed('slow')" style="flex:1">慢速</button><button class="btn-info" onclick="setScrollSpeed('fast')" style="flex:1">快速</button></div><button class="btn-purple" onclick="startScroll()">开始滚动</button><button class="btn-danger" onclick="stopScroll()">停止滚动</button></div><div class="led-matrix" id="ledMatrix"></div><div class="info">点击/滑动绘制 | 16×16矩阵</div><div class="pattern-section"><input type="text" id="patternName" class="pattern-input" placeholder="图案名称"><button class="btn-warning" onclick="savePattern()">保存</button><button class="btn-info" onclick="loadPatternsList()">刷新</button><div id="patternsList" class="pattern-list"></div></div></div><canvas id="textCanvas" style="display:none"></canvas><script>let selectedColor="#ff0000",isEraser=false,isDrawing=false;function createMatrix(){const m=document.getElementById("ledMatrix");for(let i=0;i<256;i++){const l=document.createElement("div");l.className="led";l.id="led-"+i;l.dataset.index=i;l.addEventListener("mousedown",e=>{e.preventDefault();isDrawing=true;setLed(i)});l.addEventListener("mouseenter",()=>{if(isDrawing)setLed(i)});l.addEventListener("touchstart",e=>{e.preventDefault();isDrawing=true;setLed(i)});l.addEventListener("touchmove",e=>{e.preventDefault();const t=e.touches[0],el=document.elementFromPoint(t.clientX,t.clientY);if(el&&el.classList.contains("led"))setLed(parseInt(el.dataset.index))});m.appendChild(l)}document.addEventListener("mouseup",()=>isDrawing=false);document.addEventListener("touchend",()=>isDrawing=false)}function textToMatrix(text){const canvas=document.getElementById("textCanvas");const ctx=canvas.getContext("2d");const fontSize=14;const charWidth=16;canvas.height=16;canvas.width=text.length*charWidth;ctx.fillStyle="#000";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle="#fff";ctx.font=fontSize+"px Arial,Microsoft YaHei,SimHei";ctx.textBaseline="top";ctx.fillText(text,0,1);const imageData=ctx.getImageData(0,0,canvas.width,16);const matrix=[];for(let y=0;y<16;y++){matrix[y]=[];for(let x=0;x<canvas.width;x++){const i=(y*canvas.width+x)*4;const brightness=imageData.data[i];matrix[y][x]=brightness>128?1:0}}return matrix}function startScroll(){const text=document.getElementById("scrollText").value.trim();if(!text){alert("请输入要滚动的文字");return}const textData=textToMatrix(text);fetch("/set_scroll_text",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({textData:textData})}).then(r=>r.json()).then(d=>{if(d.status==="ok"){fetch("/start_scroll",{method:"POST"}).then(r=>r.json()).then(d=>{if(d.status==="ok")alert("开始滚动")})}}).catch(e=>alert("滚动失败："+e))}function stopScroll(){fetch("/stop_scroll",{method:"POST"}).then(r=>r.json()).then(d=>{if(d.status==="ok")alert("停止滚动")}).catch(e=>alert("停止失败："+e))}function setScrollSpeed(speed){fetch("/set_scroll_speed",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({speed:speed})}).then(r=>r.json()).then(d=>{if(d.status==="ok"){const speedText=speed==="slow"?"慢速":"快速";console.log("设置滚动速度为："+speedText)}}).catch(e=>alert("设置速度失败："+e))}function selectPreset(c){selectedColor=c;isEraser=false;document.getElementById("colorPicker").value=c}function selectEraser(){isEraser=true}document.addEventListener("DOMContentLoaded",()=>{createMatrix();document.getElementById("colorPicker").addEventListener("input",e=>{selectedColor=e.target.value;isEraser=false});loadPatternsList()});function hexToRgb(h){return{r:parseInt(h.slice(1,3),16),g:parseInt(h.slice(3,5),16),b:parseInt(h.slice(5,7),16)}}function rgbToHex(r,g,b){return"#"+[r,g,b].map(x=>{const h=x.toString(16);return h.length===1?"0"+h:h}).join("")}function setLed(i){const c=isEraser?{r:0,g:0,b:0}:hexToRgb(selectedColor);fetch("/set_pixel",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({index:i,r:c.r,g:c.g,b:c.b})}).then(r=>r.json()).then(d=>{if(d.status==="ok")document.getElementById("led-"+i).style.background=isEraser?"#000":selectedColor})}function setAllLeds(){const c=hexToRgb(selectedColor);fetch("/set_all",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({r:c.r,g:c.g,b:c.b})}).then(r=>r.json()).then(d=>{if(d.status==="ok")for(let i=0;i<256;i++)document.getElementById("led-"+i).style.background=selectedColor})}function showRainbow(){fetch("/rainbow",{method:"POST"}).then(r=>r.json()).then(d=>{if(d.status==="ok"){const colors=["#f00","#ff4000","#ff7f00","#ffbf00","#ff0","#80ff00","#0f0","#00ff80","#0ff","#0080ff","#00f","#4000ff","#8000ff","#bf00ff","#f0f","#ff0080"];for(let i=0;i<256;i++)document.getElementById("led-"+i).style.background=colors[i%16]}})}function clearAll(){fetch("/clear",{method:"POST"}).then(r=>r.json()).then(d=>{if(d.status==="ok")for(let i=0;i<256;i++)document.getElementById("led-"+i).style.background="#000"})}function savePattern(){const n=document.getElementById("patternName").value.trim();if(!n){alert("请输入图案名称");return}fetch("/save_pattern",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n})}).then(r=>r.json()).then(d=>{alert(d.status==="ok"?"保存成功":"保存失败");if(d.status==="ok"){document.getElementById("patternName").value="";loadPatternsList()}})}function loadPatternsList(){fetch("/get_patterns").then(r=>r.json()).then(d=>{const l=document.getElementById("patternsList");if(d.patterns&&d.patterns.length>0){l.innerHTML=d.patterns.map(n=>`<div class="pattern-item"><span>${n}</span><div><button class="btn-success" onclick="loadPattern('${n}')">加载</button><button class="btn-danger" onclick="deletePattern('${n}')">删除</button></div></div>`).join("")}else{l.innerHTML='<div style="text-align:center;padding:15px;color:#999">暂无图案</div>'}})}function loadPattern(n){if(!confirm("确定加载 "+n+" ？"))return;fetch("/load_pattern",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n})}).then(r=>r.json()).then(d=>{if(d.status==="ok"&&d.pattern){for(let i=0;i<Math.min(d.pattern.length,256);i++)document.getElementById("led-"+i).style.background=rgbToHex(...d.pattern[i]);alert("加载成功")}else alert("加载失败")})}function deletePattern(n){if(!confirm("确定删除 "+n+" ？"))return;fetch("/delete_pattern",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n})}).then(r=>r.json()).then(d=>{alert(d.status==="ok"?"删除成功":"删除失败");if(d.status==="ok")loadPatternsList()})}</script></body></html>'''
    return html
...
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <div class="control-group">
                ...
                <div class="preset-colors">
                    <div class="preset-color eraser-btn" onclick="selectEraser()" title="橡皮擦">擦</div>
        ...
        <div class="info">
            点击或滑动绘制 | 橡皮擦熄灭 | 12秒后自动播放 | 16×16矩阵
        </div>
        
        <div class="scroll-section">
            <h3 style="margin-bottom: 10px; color: #92400e;">文字滚动播放</h3>
            <input type="text" id="scrollText" class="pattern-input" placeholder="输入要滚动的汉字或文字..." maxlength="50" style="margin-bottom: 10px;">
            <div style="display: flex; gap: 5px; margin-bottom: 10px;">
                <button class="btn-info" onclick="setScrollSpeed('slow')" style="flex: 1;">慢速</button>
                <button class="btn-info" onclick="setScrollSpeed('fast')" style="flex: 1;">快速</button>
            </div>
            <button class="btn-purple" onclick="startScroll()">开始滚动</button>
            <button class="btn-danger" onclick="stopScroll()">停止滚动</button>
        </div>
    ...
    <script>
        ...
        function textToMatrix(text) {
            ...
            canvas.height = 16;
            canvas.width = text.length * charWidth;
            ...
            // 获取像素数据
            const imageData = ctx.getImageData(0, 0, canvas.width, 16);
            const matrix = [];
        ...
        function startScroll() {
            ...
            const textData = textToMatrix(text);
            
            fetch('/set_scroll_text', {
                method: 'POST',
            ...
            .then(data => {
                if (data.status === 'ok') {
                    fetch('/start_scroll', {method: 'POST'})
                    .then(response => response.json())
                    .then(data => {
                        if (data.status === 'ok') {
                            console.log('开始滚动');
            ...
            .catch(error => alert('滚动失败：' + error));
        }
        
        // 停止滚动
        function stopScroll() {
            fetch('/stop_scroll', {method: 'POST'})
            .then(response => response.json())
            .then(data => {
                if (data.status === 'ok') {
                    console.log('停止滚动');
        ...
        function setScrollSpeed(speed) {
            ...
            .then(data => {
                if (data.status === 'ok') {
                    const speedText = speed === 'slow' ? '慢速' : '快速';
                    console.log('设置滚动速度为：' + speedText);
                }
            })
            .catch(error => alert('设置速度失败：' + error));
        }
        ...
        function showRainbow() {
            ...
            .then(data => {
                if (data.status === 'ok') {
                    const colors = [
                        '#ff0000', '#ff4000', '#ff7f00', '#ffbf00',
                        '#ffff00', '#80ff00', '#00ff00', '#00ff80',
                        '#00ffff', '#0080ff', '#0000ff', '#4000ff',
                        '#8000ff', '#bf00ff', '#ff00ff', '#ff0080'
                    ];
                    for (let i = 0; i < 256; i++) {
                        document.getElementById('led-' + i).style.background = colors[i % 16];
                    }
                }
            })
        ...
        function loadPattern(name) {
            ...
            .then(data => {
                if (data.status === 'ok' && data.pattern) {
                    for (let i = 0; i < Math.min(data.pattern.length, 256); i++) {
...
def recv_http_request(client_socket, max_bytes=50000):
    """按Content-Length接收完整HTTP请求，避免body被截断"""
    data = bytearray()
    # 先收header
    ...
    # 继续收body直到达到Content-Length
    while len(data) - body_start < content_length and len(data) < max_bytes:
        try:
            need = content_length - (len(data) - body_start)
            chunk = client_socket.recv(min(1024, need))
            if not chunk:
                break
            data.extend(chunk)
        except:
...
def handle_client(client_socket):
    """处理客户端请求"""
    global is_scrolling, scroll_text_data, scroll_speed
    ...
    try:
        ...
        # 任意非滚动接口的操作，自动停止滚动（避免覆盖用户绘制）
        if path not in ('/set_scroll_text', '/start_scroll', '/stop_scroll', '/set_scroll_speed') and is_scrolling:
            stop_scroll_text()
        ...
        elif path == '/set_scroll_text' and method == 'POST':
            try:
                data = ujson.loads(body)
                # 优先使用压缩后的columns格式（体积小）
                columns = data.get('columns', None)
                if columns is not None:
                    scroll_text_data = columns
                else:
                    # 兼容旧格式：textData（二维数组）
                    text_data = data.get('textData', [])
                    scroll_text_data = matrix_to_columns(text_data)
                is_scrolling = False
                send_response(client_socket, 'application/json', '{"status":"ok"}')
            except Exception as e:
                send_response(client_socket, 'application/json', f'{{"status":"error","message":"{str(e)}"}}')
                
        elif path == '/start_scroll' and method == 'POST':
            start_scroll_text()
            send_response(client_socket, 'application/json', '{"status":"ok"}')
            
        elif path == '/stop_scroll' and method == 'POST':
            stop_scroll_text()
            send_response(client_socket, 'application/json', '{"status":"ok"}')
            
        elif path == '/set_scroll_speed' and method == 'POST':
            try:
                data = ujson.loads(body)
                speed_mode = data.get('speed', 'slow')
                if speed_mode == 'slow':
                    scroll_speed = 1.0  # 慢速：每帧1秒（很慢，方便看清每个字）
                elif speed_mode == 'fast':
                    scroll_speed = 0.005  # 快速：每帧0.005秒（极快，每秒200像素）
                else:
                    scroll_speed = 1.0  # 默认慢速
                print(f'设置滚动速度: {speed_mode} ({scroll_speed}秒/帧)')
                send_response(client_socket, 'application/json', '{"status":"ok"}')
            except Exception as e:
                send_response(client_socket, 'application/json', f'{{"status":"error","message":"{str(e)}"}}')
...
def start_server():
    """启动Web服务器（优化版，支持自动播放和文字滚动）"""
    global last_activity_time, scroll_offset, is_scrolling, scroll_text_data
    ...
    server_socket.settimeout(0.1)  # 设置0.1秒超时，用于滚动更新
    ...
    while True:
        ...
        # 处理滚动文字
        if is_scrolling and scroll_text_data:
            current_time = time.time()
            if current_time - last_scroll_time >= scroll_speed:
                display_scroll_frame(scroll_offset)
                scroll_offset += 1
                # 如果滚动到末尾，重新开始
                text_width = len(scroll_text_data) if scroll_text_data else 0
                if scroll_offset >= text_width + 16:
                    scroll_offset = -16  # 从右侧重新进入（带尾部空白）
                last_scroll_time = current_time
        
        # 检查自动播放（仅在不滚动时）
        elif not is_scrolling and time.time() - last_activity_time > auto_play_interval:
            if saved_patterns:
                play_next_pattern()
                last_activity_time = time.time()
...
if __name__ == '__main__':
    print('========================================')
    print('ESP32 WS2812B 16x16 LED矩阵控制器 v3.3.4')
    print('========================================')
    ...
    print('  8. 亮度调节：{}% (修改BRIGHTNESS参数调整)'.format(int(BRIGHTNESS * 100)))
    print('  9. 汉字滚动播放（支持中英文混合）')
    print('')
